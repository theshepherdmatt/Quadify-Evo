...
class RotaryControl:
    ...
    def _read_encoder(self):
        ...
        try:
            ...
            self.logger.error(f"GPIO encoder read error: {e}")
            return self.last_encoded  # return last known state on failure
    ...
    def _read_button_state(self):
        ...
        try:
            ...
            self.logger.error(f"GPIO button read error: {e}")
            return GPIO.HIGH  # treat as not pressed on failure
        ...
        try:
            ...
            while True:
                ...
                if button_state == GPIO.LOW and self.button_last_state == GPIO.HIGH:
                    ...
                    while self._read_button_state() == GPIO.LOW:
                        if time.time() - press_start_time > self.long_press_threshold:
                            if self.long_press_callback:
                                self.long_press_callback()
                            break
                        time.sleep(0.01)  # avoid busy-spin while button is held
                    else:
                        if time.time() - press_start_time < self.long_press_threshold:
